...
module "mysql" {
  ...
  server_name    = "mysql-server-bgcmnged-prod"
  rg_name        = azurerm_resource_group.rg.name
  location       = var.location
...
module "managed_identity" {
  ...
  rg_name               = azurerm_resource_group.rg.name
  location              = var.location
  managed_identity_name = "managed-identity-prod"
}
...
module "appservice" {
  ...
  rg_name             = azurerm_resource_group.rg.name
  location            = var.location
  plan_name           = "fullstack-plan-prod"
  appservice_name     = "fullstack-appservice-prod"
  os_type             = "Linux"
  sku_name            = "B1"
...
module "static_web_app" {
  ...
  static_webapp_name = "fullstack-frontend-prod"
  rg_name            = azurerm_resource_group.rg.name
  location           = "eastus2"
  sku_tier           = "Free"
  sku_size           = "Free"
