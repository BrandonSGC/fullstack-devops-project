...
module "network" {
  ...
  vnet_name           = "fullstack-vnet-staging"
...
module "mysql" {
  ...
  server_name    = "mysql-server-bgcmnged-staging"
  rg_name        = azurerm_resource_group.rg.name
  location       = var.location
...
module "managed_identity" {
  ...
  rg_name               = azurerm_resource_group.rg.name
  location              = var.location
  managed_identity_name = "managed-identity-staging"
}
...
module "appservice" {
  ...
  rg_name             = azurerm_resource_group.rg.name
  location            = var.location
  plan_name           = "fullstack-plan-staging"
  appservice_name     = "fullstack-appservice-staging"
  os_type             = "Linux"
  sku_name            = "B1"
...
module "static_web_app" {
  ...
  static_webapp_name = "fullstack-frontend-staging"
  rg_name            = azurerm_resource_group.rg.name
  location           = "eastus2"
  sku_tier           = "Free"
  sku_size           = "Free"
